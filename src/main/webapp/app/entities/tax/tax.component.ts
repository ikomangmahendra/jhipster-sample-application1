...
export class TaxComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.taxService
      ...
      .subscribe(
        (res: HttpResponse<ITax[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInTaxes(): void {
    this.eventSubscriber = this.eventManager.subscribe('taxListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITax[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
